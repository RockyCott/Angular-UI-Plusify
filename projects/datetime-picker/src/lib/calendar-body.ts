 ...
 * @docs-private
 */
export class NgxPlusifyCalendarCell<D = any> {
  constructor(
    public value: number,
...
export interface NgxPlusifyCalendarUserEvent<D> {
  value: D;
  event: Event;
}
...
export class NgxPlusifyCalendarBody<D = any> implements OnDestroy, AfterViewChecked {
  ...
  rows = input<NgxPlusifyCalendarCell[][]>();
  ...
  readonly selectedValueChange = output<NgxPlusifyCalendarUserEvent<number>>();

  /** Emits when the preview has changed as a result of a user action. */
  readonly previewChange = output<NgxPlusifyCalendarUserEvent<NgxPlusifyCalendarCell | null>>();

  readonly activeDateChange = output<NgxPlusifyCalendarUserEvent<number>>();

  /** Emits the date at the possible start of a drag event. */
  readonly dragStarted = output<NgxPlusifyCalendarUserEvent<D>>();

  /** Emits the date at the conclusion of a drag, or null if mouse was not released on a date. */
  readonly dragEnded = output<NgxPlusifyCalendarUserEvent<D | null>>();
  ...
  _cellClicked(cell: NgxPlusifyCalendarCell, event: MouseEvent): void {
  ...
  _emitActiveDateChange(cell: NgxPlusifyCalendarCell, event: FocusEvent): void {
    if (cell.enabled) {
  ...
  _isComparisonBridgeStart(value: number, rowIndex: number, colIndex: number) {
    ...
    let previousCell: NgxPlusifyCalendarCell | undefined = this.rows()[rowIndex][colIndex - 1];
  ...
  _isComparisonBridgeEnd(value: number, rowIndex: number, colIndex: number) {
    ...
    let nextCell: NgxPlusifyCalendarCell | undefined = this.rows()[rowIndex][colIndex + 1];
  ...
  private _getCellFromElement(element: HTMLElement): NgxPlusifyCalendarCell | null {
